...
# json.loads()
##########################正则表达式##################################
# 正则表达式可以分两种，一种是DFA，一种是NFA，一种以表达式为主导，一种以文本为主导
# \d 用来匹配一个数字
# \w用来匹配一个字母或者数字
# .可以匹配任意字符
# \s表示空格
# *表示任意个字符（包括0个），+表示至少一个字符，？表示0或者1个字符，{n}表示n个字符，{n,m}表示n-m个字符
# {n}表示匹配确定的n次，例如o{2}不可以匹配Bob，但是可以匹配Boob
# {n,}表示至少匹配n次，例如o{2,}可以匹配Booooooob中的所有o
# {n,m}表示可以至少匹配n次，至多匹配m次
# 为了更精准的匹配，可以使用[]进行匹配
# [abc]字符集匹配，可以匹配所包含的任意一个字符，plain可以匹配出a
# [^abc]负字符集匹配，匹配为包含的任意字符集，plain可以匹配出plin
# [0-9a-zA-Z\_]可以匹配一个数字、字母或者下划线
# [^0-9a-zA-Z\_]负字符范围
# \b匹配一个单词的边界，也就是单词和空格的位置（正则表达式有两种一种是匹配字符，一种是匹配位置，\b则是匹配位置），例如，“er\b”可以匹配“never”中的“er”（结尾是er），但不能匹配“verb”中的“er”。
